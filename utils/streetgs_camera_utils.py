...
# from datasets.base_readers import CameraInfo
from scene.endo_loader import streegs_CameraInfo
...
class streetgs_Camera(nn.Module):
    def __init__(
        self, 
        id,
        R, T, 
        ...
        super(streetgs_Camera, self).__init__()
...
def loadmask(cam_info: streegs_CameraInfo, resolution, resize_mode):
    masks = dict()
...
WARNED = False
def loadCam(cam_info: streegs_CameraInfo, resolution_scale):
...
def camera_to_JSON(id, camera: streegs_CameraInfo):
    Rt = np.zeros((4, 4))
...
def make_rasterizer(
    viewpoint_camera: streetgs_Camera,
    active_sh_degree = 0,
    bg_color = None,
